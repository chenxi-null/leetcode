...
class StringToInt_atoi {
    public int myAtoi(String str) {
        ...
        String strNum = numBuilder.toString();
        if (strNum.isEmpty()) {
            return 0;
        }
        ...
        // remove leading zero
        int i = 0;
        while (i < strNum.length() && strNum.charAt(i) == '0') {
            i++;
        }
        if (i >= strNum.length()) {
            return 0;
        ...
        if (isPositive) {
            if (gte(strNum, String.valueOf(Integer.MAX_VALUE))) {
                return Integer.MAX_VALUE;
            }
        } else {
            strNum = '-' + strNum;

            if (gte(strNum, String.valueOf(Integer.MIN_VALUE))) {
